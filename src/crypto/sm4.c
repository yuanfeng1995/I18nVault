/*
 * SM4 Block Cipher (GB/T 32907-2016)
 * 128-bit key, 128-bit block, 32 rounds
 */
#include "sm4.h"

#include <string.h>

/* ---- S-Box ---- */
static const uint8_t SM4_SBOX[256] = {
    ,
};

/* ---- System parameter FK ---- */
static const uint32_t FK[4] = {
    0xA3B1BAC6,
    0x56AA3350,
    0x677D9197,
    0xB27022DC,
};

/* ---- Constant key CK ---- */
static const uint32_t CK[32] = {
    0x00070E15, 0x1C232A31, 0x383F464D, 0x545B6269, 0x70777E85, 0x8C939AA1, 0xA8AFB6BD, 0xC4CBD2D9,
    0xE0E7EEF5, 0xFC030A11, 0x181F262D, 0x343B4249, 0x50575E65, 0x6C737A81, 0x888F969D, 0xA4ABB2B9,
    0xC0C7CED5, 0xDCE3EAF1, 0xF8FF060D, 0x141B2229, 0x30373E45, 0x4C535A61, 0x686F767D, 0x848B9299,
    0xA0A7AEB5, 0xBCC3CAD1, 0xD8DFE6ED, 0xF4FB0209, 0x10171E25, 0x2C333A41, 0x484F565D, 0x646B7279,
};

/* ---- Helpers ---- */

static uint32_t get_be32(const uint8_t* b)
{
    return ((uint32_t)b[0] << 24) | ((uint32_t)b[1] << 16) | ((uint32_t)b[2] << 8) | (uint32_t)b[3];
}

static void put_be32(uint8_t* b, uint32_t v)
{
    b[0] = (uint8_t)(v >> 24);
    b[1] = (uint8_t)(v >> 16);
    b[2] = (uint8_t)(v >> 8);
    b[3] = (uint8_t)(v);
}

static uint32_t rotl32(uint32_t x, int n)
{
    return (x << n) | (x >> (32 - n));
}

/* τ: S-box substitution on each byte of a 32-bit word */
static uint32_t tau(uint32_t A)
{
    return ((uint32_t)SM4_SBOX[(A >> 24) & 0xFF] << 24) | ((uint32_t)SM4_SBOX[(A >> 16) & 0xFF] << 16) |
           ((uint32_t)SM4_SBOX[(A >> 8) & 0xFF] << 8) | (uint32_t)SM4_SBOX[A & 0xFF];
}

/* L: linear transform for encryption */
static uint32_t L(uint32_t B)
{
    return B ^ rotl32(B, 2) ^ rotl32(B, 10) ^ rotl32(B, 18) ^ rotl32(B, 24);
}

/* L': linear transform for key expansion */
static uint32_t L_prime(uint32_t B)
{
    return B ^ rotl32(B, 13) ^ rotl32(B, 23);
}

/* T = L(τ(A)) */
static uint32_t T_enc(uint32_t A)
{
    return L(tau(A));
}

/* T' = L'(τ(A)) */
static uint32_t T_key(uint32_t A)
{
    return L_prime(tau(A));
}

/* ---- Core round (shared by encrypt / decrypt) ---- */

static void sm4_rounds(const uint32_t rk[32], const uint8_t in[16], uint8_t out[16])
{
    uint32_t X[36];
    X[0] = get_be32(in);
    X[1] = get_be32(in + 4);
    X[2] = get_be32(in + 8);
    X[3] = get_be32(in + 12);

    for (int i = 0; i < 32; i++)
        X[i + 4] = X[i] ^ T_enc(X[i + 1] ^ X[i + 2] ^ X[i + 3] ^ rk[i]);

    put_be32(out, X[35]);
    put_be32(out + 4, X[34]);
    put_be32(out + 8, X[33]);
    put_be32(out + 12, X[32]);
}

/* ---- Public API ---- */

void sm4_init(sm4_ctx* ctx, const uint8_t key[SM4_KEY_SIZE])
{
    uint32_t K[36];
    K[0] = get_be32(key) ^ FK[0];
    K[1] = get_be32(key + 4) ^ FK[1];
    K[2] = get_be32(key + 8) ^ FK[2];
    K[3] = get_be32(key + 12) ^ FK[3];

    for (int i = 0; i < 32; i++)
    {
        K[i + 4]   = K[i] ^ T_key(K[i + 1] ^ K[i + 2] ^ K[i + 3] ^ CK[i]);
        ctx->rk[i] = K[i + 4];
    }
}

void sm4_encrypt_block(const sm4_ctx* ctx, const uint8_t in[SM4_BLOCK_SIZE], uint8_t out[SM4_BLOCK_SIZE])
{
    sm4_rounds(ctx->rk, in, out);
}

void sm4_decrypt_block(const sm4_ctx* ctx, const uint8_t in[SM4_BLOCK_SIZE], uint8_t out[SM4_BLOCK_SIZE])
{
    uint32_t drk[32];
    for (int i = 0; i < 32; i++)
        drk[i] = ctx->rk[31 - i];
    sm4_rounds(drk, in, out);
}
